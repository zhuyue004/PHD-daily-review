...
function noteLocalDateTime(iso){let date=new Date(iso),pad=value=>String(value).padStart(2,'0');return `${date.getFullYear()}-${pad(date.getMonth()+1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())}`}
function ensureArchiveNoteTimeInput(){let field=$('#editNoteCreatedAt');if(field)return field;field=document.createElement('label');field.id='editNoteCreatedAt';field.className='edit-note-time';field.innerHTML='<span>记录时间</span><input type="datetime-local" step="1" aria-label="随手记记录时间">';noteInput.before(field);field.querySelector('input').addEventListener('input',()=>queueNoteDraft());return field}
