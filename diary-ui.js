...
$('#preferences article:last-child p').textContent='记录保存在本设备浏览器内。保存日记或每日复盘时可请求定位，并仅保存解析后的地点名称；为将位置转为地名，会临时访问地图服务，不保存经纬度。';
async function migrateLegacyReviewLocations(){let legacy=records.filter(record=>/^-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?$/.test(record.location||''));for(let record of legacy){let [latitude,longitude]=record.location.split(',').map(Number),place=await placeFromCoordinates(latitude,longitude);if(place){record.location=place;save()}await new Promise(resolve=>setTimeout(resolve,1100))}}
