...
async function editDiary(entry){
  ...
  preview();$('#editDiaryAddImages').onclick=()=>$('#editDiaryImages').click();$('#editDiaryImages').onchange=event=>{let current=images.length-removed.length+added.length;added.push(...[...event.target.files].filter(file=>file.type.startsWith('image/')).slice(0,4-current));event.target.value='';preview()};
