...
function bindDiaryReadMore(){for(let card of $$('.diary-feed-card')){let text=card.querySelector('.diary-feed-text'),button=card.querySelector('.diary-read-more');if(!text||!button)continue;let chars=[...text.textContent.replace(/\s/g,'')].length,measure=text.cloneNode(true);measure.style.cssText=`display:block!important;position:absolute!important;visibility:hidden!important;pointer-events:none!important;left:0!important;top:0!important;width:${text.clientWidth}px!important;height:auto!important;overflow:visible!important;-webkit-line-clamp:unset!important`;card.append(measure);let overflow=chars>80||measure.getBoundingClientRect().height>text.getBoundingClientRect().height+2;measure.remove();if(overflow){button.hidden=false;button.textContent='…全文';button.onclick=event=>{event.preventDefault();event.stopPropagation();let expanded=card.classList.toggle('diary-expanded');button.textContent=expanded?'收起':'…全文'}}else button.remove()}}
