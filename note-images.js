...
async function saveDiaryImages(diaryId,files,createdAt=new Date()){let db=await imageDb(),ids=[],originals=0,stamp=imageStamp(createdAt);for(let [index,source] of [...files].entries()){let result=await compressImageForStorage(source),file=result.file;if(result.keptOriginal)originals++;let id=crypto.randomUUID(),suffix=files.length>1?`_${index+1}`:'',name=`日记${stamp}${suffix}.${imageExtension(file)}`,blob=file?.slice?file.slice(0,file.size,file.type):file;await imageRequest(db.transaction('images','readwrite').objectStore('images').put({id,noteId:diaryId,name,type:file.type||'image/jpeg',blob}));ids.push(id)}notifyOriginalImages(originals);return ids}
