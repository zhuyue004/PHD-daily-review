...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(()=>{
      ...
      observations=[{id:'obs-new-1',date:day(),title:'新观察',text:'首次正文',analysis:'我的分析',originalText:'首次正文',originalSource:'first-save',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()},{id:'obs-legacy-1',date:day(),title:'旧观察',text:'旧版正文',analysis:'',createdAt:new Date(Date.now()-60000).toISOString(),updatedAt:new Date(Date.now()-60000).toISOString()}];
    ...
    assert.strictEqual(await page.locator('.observation-compare-text').first().textContent(),'首次正文');
    ...
    assert.ok(await row.evaluate(el=>el.classList.contains('compare-swiped')),'right swipe did not reveal comparison action');
    await row.locator('.observation-compare-action').click();
    assert.strictEqual(await page.locator('.observation-compare-overlay').count(),1);
    await page.locator('.observation-compare-close').click();
    ...
    assert.match(await page.locator('.observation-compare-text').first().textContent(),/旧记录未留存首次原文/);
    await page.locator('.observation-compare-close').click();
