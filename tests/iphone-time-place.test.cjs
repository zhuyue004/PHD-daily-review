...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(()=>{notes.find(note=>note.id==='note-2').text='\\[E=mc^2\\]';saveNotes();page('archive');archive()});
    const swipeResult=await page.evaluate(()=>{
      const row=document.querySelector('.archive-note-swipe'),formula=row.querySelector('.note-display-math'),card=row.querySelector('.archive-note');
      const send=(target,type,x,y)=>target.dispatchEvent(new PointerEvent(type,{bubbles:true,pointerId:7,clientX:x,clientY:y,pointerType:'touch'}));
      send(formula,'pointerdown',200,150);send(row,'pointermove',90,150);send(row,'pointerup',90,150);
      const formulaSwiped=row.classList.contains('swiped');
      send(card.querySelector('time'),'pointerdown',200,150);send(row,'pointermove',90,150);send(row,'pointerup',90,150);
      return {formulaSwiped,cardSwiped:row.classList.contains('swiped')};
    });
    assert.deepEqual(swipeResult,{formulaSwiped:false,cardSwiped:true});
    ...
    await page.evaluate(()=>{diaries[0].place='';saveDiaries();renderDiary();return editDiary(diaries[0])});
    await page.locator('#editDiaryInput').fill('修改后的日记');
    await page.locator('#saveDiaryEdit').click();
    await page.locator('#modal').waitFor({state:'hidden'});
    assert.match((await page.evaluate(()=>diaries[0].place)),/浦东新区/);
    ...
    await page.evaluate(()=>{observations[0].place='';saveObservations();openObservationEditor(observations[0])});
    await page.locator('#saveObservation').click();
    await page.waitForFunction(()=>Boolean(observations[0]?.place));
    assert.match((await page.evaluate(()=>observations[0].place)),/浦东新区/);
