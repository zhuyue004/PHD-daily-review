...
function renderInsightPlan(bounds){
  ...
  section.querySelector('.add-plan').onclick=()=>{let item={id:crypto.randomUUID(),text:'新建条目',done:false,createdAt:planNow(),updatedAt:planNow()};entry.items.push(item);writePlan(bounds,kind,entry);renderInsightPlan(bounds);setTimeout(()=>$('#insightPlan')?.querySelector(`[data-plan-id="${item.id}"] .edit-plan`)?.click(),0)};
